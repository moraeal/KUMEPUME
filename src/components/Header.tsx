  ...
  return (
    <header id="top" className="sticky top-0 z-50 border-b border-[#183b33]/10 bg-[#fffdf8]/95 backdrop-blur-xl">
      ...
      <div className="container-page flex min-h-[78px] items-center gap-5 py-2 lg:min-h-[86px]">
        <Link to="/" className="flex shrink-0 items-center gap-3" aria-label="KUMEPUME 홈">
        ...
        <nav className="ml-auto hidden items-center gap-7 xl:flex" aria-label="주요 메뉴">
          {navigation.map((item) => (
            <div key={item.label} className="group relative py-7">
              {item.external ? (
                <a href={item.href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[14px] font-extrabold text-charcoal/72 transition hover:text-[#e47716]">{item.label}<ExternalLink size={13} /></a>
              ) : (
                <NavLink to={item.href} className={({ isActive }) => `inline-flex items-center gap-1 text-[14px] font-extrabold transition ${isActive ? "text-[#e47716]" : "text-charcoal/72 hover:text-[#e47716]"}`}>{item.label}{item.children && <ChevronDown size={14} />}</NavLink>
              )}
              {item.children && (
                <div className="invisible absolute left-1/2 top-[72px] min-w-44 -translate-x-1/2 translate-y-2 border border-[#183b33]/10 bg-white p-2 opacity-0 shadow-[0_18px_45px_rgba(24,59,51,.14)] transition group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
                  {item.children.map(([label, href]) => <Link key={href} to={href} className="block whitespace-nowrap px-4 py-2.5 text-[13px] font-bold text-charcoal/65 transition hover:bg-[#f2f7ed] hover:text-[#183b33]">{label}</Link>)}
                </div>
              )}
            </div>
          ))}
        ...
        <div className="ml-auto hidden items-center gap-2 xl:flex">
          <Link to="/join" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#ef901f] px-5 text-sm font-black text-[#183b33] transition hover:bg-[#ffc04c]"><Heart size={16} fill="currentColor" /> 함께하기</Link>
        </div>
