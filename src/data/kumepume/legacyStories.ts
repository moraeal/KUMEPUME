...
export const legacyStories: LegacyStory[] = rules.map((rule) => {
  const page = pages.find((item) => item.sourceUrl.endsWith(rule.sourcePath));
  if (!page) throw new Error(`Legacy story source missing: ${rule.sourcePath}`);
  const { mode: _mode, start: _start, ...story } = rule;
  return { ...story, sourceUrl: page.sourceUrl, paragraphs: extract(rule, page) };
});
